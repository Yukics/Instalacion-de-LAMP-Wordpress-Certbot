...
if ! [ -x "$(command -v apache2)" ]; then
  ...
  sudo ufw allow 80 && sudo ufw allow 443 && sudo ufw allow 3306 && sudo ufw allow 21 && sudo ufw allow 20
fi
...
#	</Directory>
  ...
  ServerName $nombre_usuario.com
	...
	Redirect / https://$nombre_usuario.com
...
<IfModule mod_ssl.c>
    ...
    #Si ejecutas el certbot acuerdate de editar estos dos parametros despues
    ________________________________________________________________________
...
#set WP salts
  sudo perl -i -pe'
    ...
    s/Pon lo que quieras aqui/salt()/ge
  ' wp-config.php
